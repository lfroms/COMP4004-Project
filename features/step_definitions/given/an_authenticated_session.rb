...
Given('I successfully log in as a student with email {string}') do |email|
  password = '123456'
...
Given('I successfully log in as a professor with email {string}') do |email|
  password = '123456'
  User.create(name: 'professor', email: email, password: password, admin: false, approved: true)
  visit('/')
  fill_in('login_email_field', with: email)
  fill_in('login_password_field', with: password)
  click_button('login')
