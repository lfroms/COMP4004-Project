...
Given('there exists a deliverable called {string} with due date one day ago for course offering for course with code {string} section {string} for the current term') do |title, code, section|
  course = Course.find_by(code: code)
  offering = Offering.find_by(course: course, section: section, term: @term)
